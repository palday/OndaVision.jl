...
When testing approximate equality of arrays, use `all(A .≈ B)` rather than `A ≈ B`. The latter uses a norm-based tolerance that can mask per-element discrepancies.
